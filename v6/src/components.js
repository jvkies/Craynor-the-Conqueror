...
Crafty.c('Mob', {
  ...
  //       }
	...
	giveXP: function (whom, amt) {
		...
		if (whom.curr_xp >= game.CHARLEVELS[whom.level].xp_treshold & game.CHARLEVELS[whom.level].level <= 10) {
			whom.level += 1;
...
Crafty.c('PlayerCharacter', {
  ...
  init: function() {
    this.requires('Actor, Solid, Keyboard, Collision, Mob, spr_player, SpriteAnimation')
      ...
      .reel('PlayerMovingUp',    0, 0, 2)
      .reel('PlayerMovingRight', 0, 1, 2)
      .reel('PlayerMovingDown',  0, 2, 2)
      .reel('PlayerMovingLeft',  0, 3, 2)
      .reel('PlayerLookUp',    0, 0, 0)
      .reel('PlayerLookRight', 0, 1, 0)
      .reel('PlayerLookDown',  0, 2, 0)
      .reel('PlayerLookLeft',  0, 3, 0);
  	this.isPlayer = true;
