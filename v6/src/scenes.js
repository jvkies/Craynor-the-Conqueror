...
Crafty.scene('Game', function() {
	...
	game.initPlayer(2,4);
	var stagelevel = Crafty.e('2D, Canvas, Text, Color').text("Stage: "+game.GAMELEVEL).textFont({size:'20px', family: game.GAME_FONT}).textColor('#9D9D9D').attr({ x: 748, y: 129, w: 150});
	var stage_taser = Crafty.e('2D, DOM, Text, Color, Tween').text("Stage: "+game.GAMELEVEL).textFont({size:'40px'}).css({'font-size':'40px'}).textColor('#5A1E00').attr({ x: (game.TILE_WIDTH*game.GRID_WIDTH)/2-100, y: game.TILE_HEIGHT*game.GRID_HEIGHT/2, w: 200, alpha: 0});
...
Crafty.scene('GameOver', function() {
	...
	// Crafty.e('2D, DOM, Text').text('Game over').textFont({'font-size':'45px'}).attr({ x: 0, y: game.height()/2 - 258, w: game.width() });
		...
		.text('Game over')
		.attr({ x: 0, y: game.height()/2 - 258, w: game.width() })
		.textFont({'size':'45px'});

    // .css({'font-size':'45px'});

	Crafty.e('2D, DOM, Text').text('Player Level: '+game.PLAYER.level).textFont({size:'25px'}).attr({ x: 0, y: game.height()/2 - 154, w: game.width() });
	Crafty.e('2D, DOM, Text').text('Sword Damage: '+game.PLAYER.dmg).textFont({size:'25px'}).attr({ x: 0, y: game.height()/2 - 104, w: game.width() });
	Crafty.e('2D, DOM, Text').text('Armor Defense: '+game.PLAYER.def).textFont({size:'25px'}).attr({ x: 0, y: game.height()/2 - 54, w: game.width() });
	Crafty.e('2D, DOM, Text').text('Stage Level: '+game.GAMELEVEL).textFont({size:'25px'}).attr({ x: 0, y: game.height()/2 -4, w: game.width() });
	Crafty.e('2D, DOM, Text').text('Damage Dealt: '+game.DMG_DEALT).textFont({size:'25px'}).attr({ x: 0, y: game.height()/2 +54, w: game.width() });
	Crafty.e('2D, DOM, Text').text('Enemys Killed: '+game.MOBS_KILLED).textFont({size:'25px'}).attr({ x: 0, y: game.height()/2 +104, w: game.width() });
	...
	setTimeout(function() {
		delay = false; 
		Crafty.e('2D, DOM, Text').text('press any key to restart').textFont({'size':'25px'}).attr({ x: 0, y: game.height()/2 + 204, w: game.width() });
	}, 2000);
